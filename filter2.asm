...
// **** INSERT CODE HERE ****
.BYTE4/r32 filter[]= "Wav4Filter.txt"; // [INSERTED] Imports the (32-bit) stop band filter coeficient file 
...
A1 = 0 || [I0++]= R0|| R1 = [I1++]; // [INSERTED] Rest of 72-bit acculumator register (A1) initialised to 0 (cleared), Input audio data in R0 fed into circular buffer I0 initialised before (32-bit), Filter coeficients in index register I0 are fed into R1 (32-bit) then post-increments both for the next instruction. (exploits parallelism)
...
A1:0 += R0 * R1|| R0 = [I0++] || R1 = [I1++]; // [INSERTED] Filter left by multiplying current input audio data in R0 by current filter coeficients in R1 via a 32-bit multiply into the 72-bit accumlator register A1:0. Then increments both I0 and I1 for the next multiplication. This shifted multiply accumulate (MAC) is the heart of convolution. (exploits parallelism)
LOOP_END; // [INSERTED] End of convolution loop
A1:0 += R0 * R1||I0-=4; // [INSERTED] 32-bit multiplies the final values of input audio data in R0 and filter coeficients in R1 into 72-bit accumulator register A1:0. In addition, post-decrements the I0 register by two bites (i.e. 32-bit word) in order to ensure that the next value from the codec overwrites the oldest value, not the previously most recently acquired. (exploits parallelism)
